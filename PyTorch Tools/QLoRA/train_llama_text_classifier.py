...
### TRAINING ARGUMENTS ###
##########################
experiment_name = "LlamaLoRATextClassifier"
wandb_run_name = "llama_lora_classifier"
...
target_modules = ["q_proj", "k_proj", "v_proj", "o_proj", "embed_tokens", "up_proj", "down_proj"]
exclude_modules = ["score"] # Dont do LoRA on untrained classifier
rank = 8
lora_alpha = 8
use_rslora = True
bias = "lora_only"
...
with accelerator.main_process_first():
    dataset = dataset.map(preprocess_function, batched=True, remove_columns="text", num_proc=32) 
...
trainloader = DataLoader(dataset["train"], batch_size=minibatch_size, collate_fn=data_collator, shuffle=True, num_workers=num_workers)
testloader = DataLoader(dataset["test"], batch_size=minibatch_size, collate_fn=data_collator, shuffle=False, num_workers=num_workers)
...
if use_lora:
    ...
    model = QLoraModel(model, lora_config).to(accelerator.device)
...
scheduler = get_cosine_schedule_with_warmup(optimizer=optimizer, 
                                            num_warmup_steps=warmup_steps * accelerator.num_processes / gradient_accumulation_steps, 
                                            num_training_steps=epochs * len(trainloader) * accelerator.num_processes / gradient_accumulation_steps)
...
for epoch in range(epochs):
    ...
    for batch in trainloader:
        ...
        with accelerator.accumulate(model):
            ...
            accumulated_acc += accuracy.item() / gradient_accumulation_steps
